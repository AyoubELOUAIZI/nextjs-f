const getPredictedAge = async (name: string) => {
  ...
  async function Prediction({ params }: Params) {
    ...
    return (
      <div className="max-w-md mx-auto bg-green-500 rounded-xl shadow-md overflow-hidden md:max-w-2xl m-3 p-4">
        <div className="p-8">
          <div className="uppercase tracking-wide text-sm text-indigo-500 font-semibold">
            Personal Info
          </div>
          <div className="block mt-1 text-lg leading-tight font-medium text-black">
            Age: {age?.age}  count:{age?.count}
          </div>
          <div className="block mt-1 text-lg leading-tight font-medium text-black">
            Gender: {gender?.gender}  count:{gender?.count}
          </div>
          <div className="block mt-1 text-lg leading-tight font-medium text-black">
            Nationality: {nationality?.country[0]?.country_id}   count:{nationality?.count}
          </div>
        </div>
      </div>
    );
